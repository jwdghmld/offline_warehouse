...
MYSQL_TABLE = "offline.ads_sku_topn_df"
...
def main():
    parser = argparse.ArgumentParser(description="发布 SKU 排行到 MySQL")
    ...
    spark = (
        SparkSession.builder.appName("publish_ads_sku_topn")
    ...
    try:
        ...
        if (
            ...
            raise ValueError(
                f"SKU 排行校验失败：ds={args.ds}, rows={source_count}, "
                f"keys={key_count}, non_null_rows={non_null_count}"
            )
        if source.where(
            "stat_period not in ('1d', '7d', '30d', 'all') "
            "or rank_type not in "
            "('gmv', 'paid_sku_num', 'paid_order_count') "
            "or rank_no < 1 or rank_no > 10 or sku_id is null "
            "or paid_order_count < 0 or paid_user_count < 0 "
            "or paid_sku_num < 0 or gmv < 0"
        ).take(1):
            raise ValueError("SKU 排行存在非法字段")
        ...
        check = spark.read.jdbc(
            ...
            f"count(distinct stat_period, rank_type, rank_no) as key_count "
            f"from {MYSQL_TABLE} where ds='{args.ds}') publish_check",
            properties=MYSQL_PROPERTIES,
        ).first()
        if check["row_count"] != source_count or check["key_count"] != source_count:
            raise ValueError("SKU 排行发布结果不完整")

        print(f"SKU 排行发布完成：ds={args.ds}, rows={source_count}")
    finally:
        spark.stop()
