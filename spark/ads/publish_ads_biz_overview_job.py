...
MYSQL_TABLE = "offline.ads_biz_overview_df"
...
def main():
    parser = argparse.ArgumentParser(description="发布经营总览到 MySQL")
    ...
    spark = (
        SparkSession.builder.appName("publish_ads_biz_overview")
    ...
    try:
        ...
        source = spark.sql(f"""
            select
              ds, stat_period, pv_count, uv_count, favorite_count, cart_count,
              order_count, paid_order_count, paid_user_count, paid_sku_num,
              gmv, avg_order_amount, view_to_pay_rate
            from ads.ads_biz_overview_df
            where ds='{args.ds}'
        """).cache()
        source_count = source.count()
        key_count = source.select("ds", "stat_period").distinct().count()
        ...
        if (
            ...
            raise ValueError(
                f"经营总览校验失败：ds={args.ds}, rows={source_count}, "
                f"keys={key_count}, non_null_rows={non_null_count}"
            )
        if source.where(
            "stat_period not in ('1d', '7d', '30d', 'all') "
            "or gmv < 0 or avg_order_amount < 0 "
            "or view_to_pay_rate < 0 or view_to_pay_rate > 1"
        ).take(1):
            raise ValueError("经营总览存在非法字段")
